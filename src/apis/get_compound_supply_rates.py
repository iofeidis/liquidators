...
def borrow_rates(asset_name: str, asset_address: str,
    ...
    for t in tqdm(range(start_timestamp, end_timestamp, 2000000)):
        ...
        params = {
            ...
            "max_block_timestamp": min(end_timestamp, t + 2000000),
            "num_buckets": buckets,
        }
        ...
        if "supply_rates" in resp.keys() and resp["supply_rates"]:
            d = resp["supply_rates"]
            ...
            df.to_csv(f"results/rates/{asset_name}_supply_rates_{k}.csv", index=False)
        else:
            continue
...
if __name__ == "__main__":
    ...
    for asset_name, asset_address in assets.items():
        borrow_rates(asset_name, asset_address, START_DATE, END_DATE, k=1)
